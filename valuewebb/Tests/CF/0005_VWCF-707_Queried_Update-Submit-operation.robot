*** Settings ***
Documentation    Test Case 005 VWCF-707. This Test Case is used for to update and submit Queried Transactions
...
${TC005_ALERT_MESSAGE_UPDATED}      Update operation is done. The status of the document is Queried
${TC005_ALERT_MESSAGE_SUBMITTED}    Submit operation is done. The status of the document is Submitted
${TC005_TT_STATUS}          Queried
${TC005_WAITING_TIMEOUT}    20s


*** Test Cases ***
Test Case 005 VWCF-707 Verify Update/Submit operation upon Queried status of transaction
    [Tags]    Smoke
    Given Login at ERC Homepage
    When User to change the French Translation to English
    And In Search page, open for Queried status of transaction by clicking Eye button
...
In Search page, open for Queried status of transaction by clicking Eye button
...
Open again the recently updated transaction
    [Documentation]     This keyword is to open again the recently updated transaction
    Searched for the newly created TT ID by entering TT ID on TT field then hit Search button
    SearchTT_Requests_Page.Click Edit Icon button

Searched for the newly created TT ID by entering TT ID on TT field then hit Search button
   Wait Until Element is Visible    xpath=${ERC_DOCPAGE_TTID_LOCATOR}       timeout=${TC005_WAITING_TIMEOUT}
