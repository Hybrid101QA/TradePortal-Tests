...
...              using Export Import XML operation exist only in QA environment
...
*** Variables ***
${TC010_IMPORT_XML_FULLPATH}   ${EXECDIR}/Results/TT*.xml
...
Test Case 010 SWERCBJ-152 Verify Store Status Creation Export and Import XML
    ...
    And Search and open for AVD Issued status transaction by clicking the Eye button
    And Click Export XML button
    ...
    And Click on Import XML button
...
Search and open for AVD Issued status transaction by clicking the Eye button
...
User to validate all required fields has been correctly supplied
    ...
    Wait Until Page Contains Element  xpath=${ERC_DOCPAGE_INPUT_PACKNO}   timeout=${TC010_TC_TIMEOUT}
    TT_Document_Page.Enter/Modify Field Details of     xpath=${ERC_DOCPAGE_INPUT_PACKNO}   ${TC010_PACKAGE_NO}
    Wait Until Element is Visible     xpath=${ERC_DOCPAGE_SUGGEST_PACKTYPE}   timeout=${TC010_TC_TIMEOUT}
    Wait Until Element is Enabled     xpath=${ERC_DOCPAGE_SUGGEST_PACKTYPE}   timeout=${TC010_TC_TIMEOUT}
    TT_Document_Page.Enter/Modify Field Details of    xpath=${ERC_DOCPAGE_SUGGEST_PACKTYPE}    ${TC010_PACKAGE_TYPE}
