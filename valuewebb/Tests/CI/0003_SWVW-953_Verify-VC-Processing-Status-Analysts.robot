*** Settings ***
Documentation    Test Case 003 SWVW-953. This Test Case is used for Import and Export of .xls file
...
${TC003_XLS_DOWNLOAD}   TT*.xls
${TC003_IMPORT_XLS_FULLPATH}      ${EXECDIR}/Results/TT*.xls
${TC003_IMPORT_XLS_REF_VALUE}     //p[contains(text(),'Successfully Imported')]
${TC003_TT_STATUS}      Processing


*** Test Cases ***
Test Case 003 SWVW-953 Verify V&C Processing status export and import .xls operation
    [Tags]    Smoke
...
Verify the Items indicated in the table then click Export Items button
    ...
    TT_Document_Page.Wait File Download to Finish   ${TC003_XLS_DOWNLOAD}   ${TC003_DOWNLOADS_DIRECTORY}
    TT_Document_Page.File Should Exists in Downloads Directory  ${TC003_XLS_DOWNLOAD}   ${TC003_DOWNLOADS_DIRECTORY}

User to Click Import Items
    [Documentation]      Test Step 5 Click Import XLS
    TT_Document_Page.Import XLS File from   ${TC003_IMPORT_XLS_FULLPATH}
    Wait Until Page Contains Element    ${TC003_IMPORT_XLS_REF_VALUE}    timeout=${ERC_DOCPAGE_WAITING_TIMEOUT}
